...
router.post('/getData', function(req, res, next) {
    ...
    mysqlDatabase.query('select * from novel where type like "%'+searchOptions.type+'%" and title like "%'+searchOptions.title+'%" and author like "%'+searchOptions.author+'%" limit '+offset+' , '+pageOptions.pageSize, [],
                        function(error, results, fields){
        let returnAllData=[];
        if(error) {
            throw error;
            ...
            for(let i=0;i<results.length;i++){
                ...
                fs.readFile("./novel/第一篇.txt",'utf-8',function(err,data){  
                    if(err){  
                        console.error(err);  
                    }  
                    else{  
                        ...
                        console.log("returnItemData.content"+content);
